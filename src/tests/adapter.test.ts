...
suite('Node Debug Adapter', () => {
	...
	setup( () => {
		dc = new DebugClient('node', DEBUG_ADAPTER, 'graalvm');
		return dc.start();
	});
	...
	suite('launch', () => {
		...
		test('should run program to the end', () => {
			...
			const PROGRAM = Path.join(DATA_ROOT, 'test.4gl');
	...
	suite('setBreakpoints', () => {
		...
		test('hitting a lazy breakpoint should send a breakpoint event', () => {
			...
			const PROGRAM = Path.join(DATA_ROOT, 'testLazyBreakpoint.4gl');
	...
	suite('setExceptionBreakpoints', () => {
		...
		test('should stop on an exception', () => {
			...
			const PROGRAM_WITH_EXCEPTION = Path.join(DATA_ROOT, 'testWithException.4gl');
